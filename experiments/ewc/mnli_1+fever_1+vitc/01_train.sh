...
# Specify prior model with lambda
prior_best_lambda=$(cat "../${prior}/lambda_best.txt")
prior_model_dir="../${prior}/${pretrained}-${max_len}-${prior_best_lambda}-mod"

# Specify model with lambda
model_dir="${pretrained}-${max_len}-${lambda}-mod"

unset -v ckpt
for file in "${prior_model_dir}/checkpoints"/*.ckpt; do
